#!/usr/bin/env bash
# Cuts an OpenTunnel release: bump VERSION, verify, tag, publish, reopen
# development. Adapted from the 1.x release script for the v2 shell tooling.
#
#   scripts/release.sh [patch|minor|major]
#
# With no argument the bump is derived from the commit subjects since the
# latest tag.
...
ensure_release_tools() {
	...
	command -v bats >/dev/null 2>&1 || die "bats is required"
}
...
run_verification() {
	shellcheck -s bash scripts/*.sh build/*.sh test/e2e.sh &&
		bats test/unit &&
		OPENTUNNEL_SKIP_CHECKSUMS=1 build/embed.sh &&
		bash -n dist/host.sh &&
