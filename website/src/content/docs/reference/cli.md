...
The `opentunnel` binary has three subcommands. In normal use you never install it: the `/cli` bootstrapper downloads a temporary, checksum-verified copy and passes your arguments through, so `curl -fsSL https://opentunnel.sh/cli | sh -s -- create` and `opentunnel create` are equivalent.
