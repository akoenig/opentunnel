...
- `OPENTUNNEL_KEEP_AUDIT` creates its copy with noclobber and umask 077 rather than `cp`, because the target name is predictable to the second and `cp` follows a symlink planted at it.
- Two claims in section 14.1 were wrong and the copy now says so: the wrapper refuses a session with no command, but a client that asks for a PTY still gets one (tailcat grants it), and the audit log records the command line of a session, not what that command then does (`remote bash` with a script on stdin is one line). Both verified against a live session.
