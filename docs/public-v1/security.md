...
The host and client establish an end-to-end encrypted secure channel using invite material generated by the host. The relay sees the session routing identifier from the invite and encrypted frames. It should not see the full invite code, client secret, host public key, commands, stdout, stderr, remote exit codes, or host metadata.
